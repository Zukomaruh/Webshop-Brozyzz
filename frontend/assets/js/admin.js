$(document).ready(function () {
            ...
            success: function (response) {
                if (response.success) {
                    ...
                    response.data.forEach(function (customer) {
                        ...
                        let disableBtnAttr =
                            customer.role === "admin" ? "disabled" : "";

                        let deactivateBtnText =
                            customer.status === "active"
                                ? "Deactivate"
                                : "Activate";

                        let deactivateBtnClass =
                            customer.status === "active"
                                ? "btn-outline-danger"
                                : "btn-success";
                            ...
                            <tr>
                                ...
                                <td class="text-end">
                                    <div class="btn-group gap-1">
                                        ...
                                        <button
                                            class="btn btn-sm btn-info text-white btn-view-customer"
                                            data-id="${customer.user_id}">
                                            View Details
                                        </button>

                                        <button
                                            class="btn btn-sm btn-warning btn-view-orders"
                                            data-id="${customer.user_id}">
                                            View Orders
                                        </button>

                                        <button
                                            class="btn btn-sm ${deactivateBtnClass} btn-toggle-status"
                                            data-id="${customer.user_id}"
    ...
    $(document).on("click", ".btn-toggle-status", function () {
        ...
        if (
            confirm(
                `Are you sure you want to set this customer to ${newStatus}?`
            )
        ) {
            $.ajax({
                type: "POST",
